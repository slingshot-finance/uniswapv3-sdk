...
func TestComputePoolAddress(t *testing.T) {
	...
	result, err := ComputePoolAddress(factoryAddress, tokenA, tokenB, uint64(constants.FeeLow), "")
	if err != nil {
		panic(err)
	}
	...
	resultA, err := ComputePoolAddress(factoryAddress, USDC, DAI, uint64(constants.FeeLow), "")
	if err != nil {
		panic(err)
	}
	resultB, err := ComputePoolAddress(factoryAddress, DAI, USDC, uint64(constants.FeeLow), "")
	if err != nil {
		panic(err)
	}
