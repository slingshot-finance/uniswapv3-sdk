...
func initPool() (*Pool, int, int) {
	...
	tickSpacing := constants.TickSpacings[uint64(constants.FeeLow)]
	p, err := NewPool(DAI, USDC, uint64(constants.FeeLow), poolSqrtRatioStart, big.NewInt(0), poolTickCurrent, nil)
	if err != nil {
		panic(err)
	}
...
func TestMintAmountsWithSlippage(t *testing.T) {
	...
	minPricePool, err := NewPool(DAI, USDC, uint64(constants.FeeLow), utils.MinSqrtRatio, big.NewInt(0), utils.MinTick, nil)
	assert.NoError(t, err)
	...
	maxPricePool, err := NewPool(DAI, USDC, uint64(constants.FeeLow), new(big.Int).Sub(utils.MaxSqrtRatio, big.NewInt(1)), big.NewInt(0), utils.MaxTick-1, nil)
	assert.NoError(t, err)
